...
class CreateKaggleSurveyDB:
    ...
    for survey_year in survey_years:
        ...
        df = pd.read_csv(file_path,low_memory=False,skiprows=[1]).iloc[:, 1:]
        df_dict[survey_year,"responses"] = df
        df = pd.read_csv(file_path,nrows=1)
        question_descriptions = df.values.ravel()[1:]
    ...
    def tidy_2020_2021_data(self, survey_year:int) -> tuple:
        ...
        response_df_resetindex = response_df.reset_index()
        response_df_melted = pd.melt(response_df_resetindex, id_vars='index', var_name='question_index', value_name='responses')
